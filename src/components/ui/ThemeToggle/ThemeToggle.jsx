...
export const ThemeToggle = ({ className, ...props }) => {
  const [theme, setTheme] = useState(() => {
    ...
    return 'dark'; // Always default to dark theme matching pentabytelabs.com
  });
  ...
  return (
    <button
      ...
      type="button"
      className={clsx(
        'p-2 rounded-lg text-[var(--muted-foreground)] hover:text-[var(--foreground)] hover:bg-[var(--muted)]/50 transition-all duration-300 focus:outline-none cursor-pointer flex items-center justify-center opacity-70 hover:opacity-100',
        className
      )}
